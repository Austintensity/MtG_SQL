...
def make_list(string_in):
    ...
    if split_string[len(split_string) - 1] == "":
        split_string.pop()
    return split_string
...
def rebuild_list(list_to_fix):
    """Parameter:   list_to_fix:  A list that was broken down character by character to put back.
    Returns:  String 'rebuildstring'
    """
    rebuildstring = ""
...
def target_search(oracletext, card_type, keywords):
    """REGEX oracle text search check for usual 'target' lexicon, then goes through list of target types to see if
    those were found in/around the 'target' text.
    Parameters: oracletext:  Cards oracle text, having run through check_oracle_text()
                card_type:   Card type from MtgCards/MTG_Face_Cards   {'Creature', 'Sorcery', etc}
                keywords:    Keywords for a card, to check for 'Hexproof' and 'Shroud' which would have "target" be
                in the explanation text anyway
    Returns:  list: 'has_target'
    **This was partially on the MTG_Regex page under regex_get_target() but I was having issues so it was all combined
    and moved here

     For the time being, this won't take into consideration anything that wouldn't be effected by permanents with
    hexproof or shroud, since they aren't 'targeting' a creatumtg_sql.insert_   Though this could be adapted to also
    include cards with 'Choose' in the text.  However some are trickier to find still...
    
    ['You may have Clone enter the battlefield as a copy of any creature on the battlefield']  - Doesn't technically
    target a creature, but similar effect"""

    if 'target' in oracletext.lower():
        # has_target = regex_get_target(oracletext, card_type, keywords)
        target = ""
        keywords = rebuild_list(keywords)
        regex_target_terms = ['destroy', 'damage', 'return', 'exile', 'flip', 'discard', 'shuffle', 'enchant', 'haunt',
                              'reveal', 'exchange', 'copy', 'life', 'draw', 'attach', 'copies', 'attack', 'block',
                              'sacrifice', 'token']
        ...
        if mtg_regex.re.findall(mtg_regex.regex_etb_modes['mill'], oracletext, mtg_regex.re.IGNORECASE):
            target = target + "Mill "
        ...
        if "can't be the target of spells or abilities" in oracletext.lower() or \
                "can't be the targets of spells or abilities" in oracletext.lower():
            target = target + "Shroud "
        if 'shroud' in oracletext.lower():
            target = target + "Shroud "
        if 'hexproof' in oracletext.lower():
            target = target + "Hexproof "
        if 'mutate' in keywords.lower():
            target = target + "Mutate "
        if 'heroic' in keywords.lower():
...
def ramp_search(oracletext, card_type):
    """Runs a basic oracle text search to see if it is possibly a ramp card.  If so, then runs regex_get_ramp() to
    analyse further.
    *Lands don't count as ramp cards
    ...
    has_ramp = ''
    """[{T}, Sacrifice a black or artifact creature: Add an amount of {B} equal to the sacrificed creature's converted 
    mana cost.]: Soldevi Adnate IS a ramp creature [{R}:  Add {B} to your mana pool ] is not a ramp creature, just a 
    mana-fixing one.. pretty rare exception though]:  Agent of Stromgald 
    plus all those cards that provide treasure tokens or Eldrazi Scion that sacrifice for mana aren't exactly ramp cards
    either """
    land_names = ['land', 'plains ', 'island ', 'mountain ', 'swamp ', 'forest ']
    search_ramp_terms = ['add {c}', 'add {w}', 'add {u}', 'add {b}', 'add {r}', 'add {g}', 'add {1}', 'add {2}',
                         'add {3}', 'add {4}', 'add one mana', 'add two mana', 'add three mana', 'your mana pool',
                         'add one mana']

    for terms in search_ramp_terms:
        if terms in oracletext or terms in oracletext.lower():
            has_ramp = 'TRUE'

    if 'onto the battlefield' in oracletext.lower() or 'into play' in oracletext:
        for lands in land_names:
            if lands in oracletext.lower():
                has_ramp = 'TRUE'
    ...
    if (len(ramp_abilities) == 0 and 'treasure token' in oracletext) or (ramp_abilities == "" and 'create a treasure'
                                                                         in oracletext):
        # print ("CAPTURED TREASURE!  Converted [] ramp cards back to ['Treasure']")
        ramp_abilities = ['Treasure']
    if ramp_abilities == "":
        ramp_abilities = "Other"
...
def wipe_search(oracletext, keywords):
    """Runs a basic oracle text search to see if it is possibly a board wipe card.  If so, then runs regex_get_wipe() 
    to analyse further
    Returns:  list   ex ['Shuffle', 'Exile']  """
    has_wipe = ''
    search_wipe_terms = ['destroy all', 'exile all', 'destroy each', 'exile each', 'return each', 'sacrifices the rest',
                         'destroy the rest', 'damage to each', 'all other', 'each other', ' every ']
    keywords = rebuild_list(keywords)
...
def trigger_search(oracletext, keywords):
    ...
    for terms in triggered_ability_terms:
        if terms in oracletext or terms in oracletext.lower(): 
            triggered_ability = mtg_regex.regex_get_triggered(oracletext, keywords)
...
def activated_search(oracletext, keywords):
    ...
    for terms in activated_ability_terms:
        if terms in oracletext or terms in oracletext.lower():
            activated_ability = mtg_regex.regex_get_activated(oracletext, keywords)
...
def legendary_search_faces(face1_type, face2_type):
    ...
    for terms in legend_terms:
        if terms in face1_type.lower():
            is_legend = 'Y:'
        if terms in face2_type.lower():
            foundflag = 'Y'
...
def land_handle(name, oracletext, clr_id):
    ...
    Parameters:   name:  Official card name
            oracletext:  Oracle text of the card, having run through check_oracle_text()
                :MtgGuiclr_id:  card's color identity, (counts front and back)  ex  ['B', 'G', 'U']
    ...
    for findmodes in mtg_regex.regex_searchmode:
        if 'mana' in findmodes:
            ...
            if found_mana is None:
                ...
                try:
                    use_mana = convert_manawords[found_mana]
                finally:
                    use_mana = found_mana
            ...
            if use_mana == 'X' or "add x " in oracletext.lower() or " x mana" in oracletext.lower():
                ...
                break
            try:
                if use_mana > greatest_mana:
                    greatest_mana = use_mana
            finally:
                if int(use_mana) > greatest_mana:
                    greatest_mana = int(use_mana)
...
def planeswalker_handle(name, oracletext, loyalty):
    ...
    if loyalty != 'X' and cost_ability != 'X':
        try:
            if int(cost_ability) > int(loyalty):
                ultimate_ability = 'TRUE'
        except Exception as e:
            print("Exception triggered: ", e, type(cost_ability), cost_ability,
                  " loyalty: ", type(loyalty), loyalty)
            if cost_ability > loyalty:
                ultimate_ability = 'TRUE'

    high_cost_ability = "-" + str(high_cost_ability)
    if high_cost_ability == "-0": 
        high_cost_ability = "-"
    ...
    """The number of boost abilities and cost abilities will determine how it gets added to the table w 
    insert_planeswalker().
    ...
    if boost_count == 0:
        ...
        # has 1 or more boost abilities and 1 cost ability, if it has 2 boost abilities, than the cost ability is
...
def import_inventory():
    ...
    populate the 'Cards'/'Cards_Condensed' and 'Face_Cards'/'Face_Cards_Condensed', and subsequently the 'Creatures',
    'Lands', 'Spells','Planeswalkers' tables get filled as well.
    Timed process, with scryfall query delay setting of .05, full inventory import takes approximately ...
    55 min 52 sec (8-24-20218)
    ...
    # latest_inventory = data_folder / 'MTG Inventory 21-8-18.csv'  # this is the same is the 21-7-15.csv file but
    # has bosium strip and lim-dul's crap removed
    ...
    integrating scryfall will ultimately save a lot of time but I won't want to leave the system to do 1000+ queries 
    at once
...
def single_sided_card(the_card):
    ...
    Creates instance of MtgCards() class
    Determines  keywords, {'etb', 'ramp', 'draw', 'target', 'wipe', 'activated' , 'triggered'} abilities for single
    sided cards
    """
    card_name = the_card.name()
    search_card = MtgCards(card_name, the_card.layout(), the_card.color_identity(), the_card.mana_cost(),
                           mtg_regex.regex_get_cmc(the_card.mana_cost()), the_card.type_line(), "", 'No', 'etb',
                           'ramp', 'draw', 'target', 'wipe', 'activated', 'triggered', 'image_uri',
                           check_oracle_text(the_card.oracle_text()))

    if not search_card.clr_id:
        search_card.clr_id.append('C')
    ...
    if "legendary" in card_types.lower():
        search_card.is_legendary = 'Yes'
    ...
    if 'Land' in search_card.c_type:
        land_handle(card_name, card_string, the_card.color_identity())
    if 'Planeswalker' in search_card.c_type:
        planeswalker_handle(search_card.name, search_card.oracle_text, the_card.loyalty())
    if 'Creature' in search_card.c_type:
        mtg_sql.insert_creature(card_name, sub_type, search_card.cmc, the_card.power(), the_card.toughness(),
                                search_card.keywords)
    if 'sorcery' in search_card.c_type.lower() or 'instant' in search_card.c_type.lower():
        spell_handle(search_card.name, search_card.oracle_text, search_card.c_type)

    oracletext = card_string.lower()
    # This is only converted to lower case for checking oracle text for certain keywords
    ...
    mtg_sql.insert_card(card_name, search_card.clr_id, search_card.mana_cost, search_card.cmc, search_card.c_type,
                        search_card.keywords, search_card.is_legendary, search_card.has_ETB, search_card.has_ramp,
                        search_card.has_draw, search_card.has_target, search_card.has_wipe, search_card.has_activated,
                        search_card.has_triggered, search_card.image_url, search_card.oracle_text)


def split_card(the_card):
    """Creates instance of MtgFaceCards() class and passes it and the details of card being searched to handler
    Returns: nothing
    """
    cardfaces = the_card.card_faces()
    card1 = cardfaces[0]
    card2 = cardfaces[1]
    search_card = MtgFaceCards(card1['name'], the_card.layout(), card2['name'], the_card.color_identity(),
                               card1['mana_cost'], card2['mana_cost'], mtg_regex.regex_get_cmc(card1['mana_cost']),
                               mtg_regex.regex_get_cmc(card2['mana_cost']), card1['type_line'], card2['type_line'],
                               "", "", 'F:F', "etb1", "etb2", "ramp1", "ramp2", "draw1", "draw2", "target1", "target2",
                               "wipe1", "wipe2", 'activated1', 'activated2', "triggered1", "triggered2",
                               the_card.image_uris()['normal'], the_card.image_uris()['normal'],
                               check_oracle_text(card1['oracle_text']), check_oracle_text(card2['oracle_text']))
...
def double_faced_card(the_card):
    """Creates instance of MtgFaceCards() class and passes it and the details of card being searched to Handler,
    (slightly different than for split cards that have only 1 card image)
    Returns: nothing
    """
    cardfaces = the_card.card_faces()
    card1 = cardfaces[0]
    ...
    search_card = MtgFaceCards(card1['name'], the_card.layout(), card2['name'], the_card.color_identity(),
                               ...
                               "", "", "F:F", "etb1", "etb2", "ramp1", "ramp2", "draw1", "draw2", "target1", "target2",
                               "wipe1", "wipe2", 'activated1', 'activated2', "triggered1", "triggered2",
                               card1['image_uris']['normal'], card2['image_uris']['normal'],
                               check_oracle_text(card1['oracle_text']), check_oracle_text(card2['oracle_text']))
...
def handle_double(the_card, search_card):
    ...
    Parameters:  'the_card': The scryfall card object
                'search_card': Our instance of MtgFaceCards() class
    Returns nothing  RUNS insert_face_card() to add to 'Face_Cards' and 'Face_Cards_Condensed' tables
    
    Determines keywords, {'etb', 'ramp', 'draw', 'target', 'wipe', 'activated' , 'triggered'} abilities for split
    ...
    if keywords.count("'") > 2:
        ...
        if use_this_str.lower() in search_card.face1_oracle_text.lower():
            search_card.face1_keywords = keywords
        if use_this_str.lower() in search_card.face2_oracle_text.lower():
            search_card.face2_keywords = keywords

    if 'Land' in search_card.face1_type:
        land_handle(search_card.name, search_card.face1_oracle_text, search_card.clr_id)

    if 'Land' in search_card.face2_type:
        land_handle(search_card.flipname, search_card.face2_oracle_text, search_card.clr_id)

    if 'Planeswalker' in search_card.face1_type:
        planeswalker_handle(search_card.name, search_card.face1_oracle_text, card1['loyalty'])
    ...
    if 'Creature' in search_card.face1_type:
        if '-' in search_card.face1_type:
            ...
            sub_type = card_types[1]
        mtg_sql.insert_creature(search_card.name, sub_type, search_card.face1_cmc, card1['power'], card1['toughness'],
                                search_card.face1_keywords)
    ...
    if 'Creature' in search_card.face2_type:
        ...
        mtg_sql.insert_creature(search_card.flipname, sub_type, search_card.face2_cmc, card2['power'],
                                card2['toughness'], search_card.face2_keywords)
    ...
    search_card.face1_target = target_search(search_card.face1_oracle_text.lower(), search_card.face1_type.lower(),
                                             search_card.face1_keywords.lower())
    search_card.face2_target = target_search(search_card.face2_oracle_text.lower(), search_card.face2_type.lower(),
                                             search_card.face2_keywords.lower())
    ...
    search_card.face1_ETB = etb_search(search_card.face1_oracle_text.lower(), search_card.face1_type.lower(),
                                       search_card.face1_keywords.lower())
    search_card.face2_ETB = etb_search(search_card.face2_oracle_text.lower(), search_card.face2_type.lower(),
                                       search_card.face2_keywords.lower())
    ...
    # name, layout, flipname, clr_id, mana_cost, cmc, c_type, keywords, is_legend, has_etb, has_ramp, has_draw, 
    # has_target, has_wipe, has_activated, has_triggered, image_url, oracle_text
    mtg_sql.insert_face_card(search_card.name, search_card.layout, search_card.flipname, search_card.clr_id,
                             search_card.face1_cost, search_card.face1_cmc, search_card.face1_type,
                             search_card.face1_keywords,
                             search_card.is_legendary, search_card.face1_ETB, search_card.face1_ramp,
                             search_card.face1_draw, search_card.face1_target, search_card.face1_wipe,
                             search_card.face1_activated,
                             search_card.face1_triggered, search_card.face1_image_url, search_card.face1_oracle_text)

    mtg_sql.insert_face_card(search_card.flipname, search_card.layout, search_card.name, search_card.clr_id,
                             search_card.face2_cost, search_card.face2_cmc, search_card.face2_type,
                             search_card.face2_keywords,
                             search_card.is_legendary[::-1], search_card.face2_ETB, search_card.face2_ramp,
                             search_card.face2_draw, search_card.face2_target, search_card.face2_wipe,
                             search_card.face2_activated,
...
def scryfall_search_exact(card_name):
    """Queries the scryfall server and gets the relevant card info, and based on the card layout it extracts the data 
    accordingly
    ...
    image_uris  -  this returns a dictionary containing links for 'small', 'normal', 'large', 'png', 'art_crop', 
    'border_crop'  .. based on .. which set?
            'small' is ~ 145 x 200     'normal' is ~ 488 x 680 - this fits to screen on my laptop.    'large' is ~ 
            670 x 930 - also fits to screen
...
def main():
    ...
    # import_inventory()   ## - This should be accessed through the gui instead, so you can specify which file to
    # open, but the option is available

    # SEARCH FUNCTIONS
    # scryfall_search_exact ("Oath of Lim-Dul")
    # quick_test_search("Arni Slays the Troll")

    # Run this block to rebuild all my decklists from the last exported text files in the \decks folder
    # mtg_sql.delete_all_zdecktables()    
    # mtg_sql.import_all_decks()
    # mtg_sql.create_all_decks()

    """ MAIN PROGRAM START"""

    # Build the GUI (tkinter) window and initialize the list of saved decks
    d = gui_class.MtgGui(gui_class.root)
    d.fill_decklist()
    d.new_deck()
