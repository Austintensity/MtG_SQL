...
class MTG_GUI:
    ...
    def build_deck_selector(self, window):
        #### DECK SELECTOR START ####  ## Deck Selection Frame on the left side and A Card Finder Frame behind which it may hide ##
        ...
        ## Mana Colour Buttons
        self.deck_white = IntVar()
        self.deck_blue = IntVar()
        self.deck_black = IntVar()
        self.deck_red = IntVar()
        self.deck_green = IntVar()
        self.deck_colourless = IntVar()
        
        # Check Buttons to filter which decks to display in the deck list
        self.checkwhite_d = Checkbutton(self.deck_select,image=whiteimage,variable=self.deck_white,command=self.show_certain_decks,borderwidth=0)
        self.checkwhite_d.place(x = 0, y = 380)#, width = 40)
        self.checkblue_d = Checkbutton(self.deck_select,image=blueimage,variable=self.deck_blue,command=self.show_certain_decks,borderwidth=0)
        self.checkblue_d.place(x = 60, y = 380)
        self.checkblack_d = Checkbutton(self.deck_select,image=blackimage,variable=self.deck_black,command=self.show_certain_decks,borderwidth=0)
        self.checkblack_d.place(x = 120, y = 380)
        self.checkred_d = Checkbutton(self.deck_select,image=redimage,variable=self.deck_red,command=self.show_certain_decks,borderwidth=0)
        self.checkred_d.place(x = 180, y = 380)
        self.checkgreen_d = Checkbutton(self.deck_select,image=greenimage,variable=self.deck_green,command=self.show_certain_decks,borderwidth=0)
        self.checkgreen_d.place(x = 240, y = 380)
        self.checkcolourless_d = Checkbutton(self.deck_select,image=colourlessimage,variable=self.deck_colourless,command=self.show_certain_decks,borderwidth=0)
    ...
    def build_deck_details(self, window):
        ...
        self.decktierbox = OptionMenu(self.deck_details, self.deck_tier, *self.deck_tier_options, command = self.get_deck_tier)
        ...
        self.deckstatusbox = OptionMenu(self.deck_details, self.deckstatus, *self.deck_status_options, command = self.get_deck_status)#"ACTIVE", "INACTIVE", "UNASSEMBLED")
    ...
    def build_card_finder(self, window):
        ...
        self.checkcolourless = Checkbutton(self.card_finder,image=colourlessimage,variable=self.inc_colourless,borderwidth=0)
        self.checkcolourless.place(x = 235, y = 100)
        self.checkcolourless = Checkbutton(self.card_finder,text="Exclusive?",variable=self.exclusive_colour,borderwidth=0)
        ...
        ## SQL Statement
        self.sql_label = ttk.Label(self.card_finder, text = 'OR.. enter SQL parameters: SELECT\nName, Mana_Cost, Card_Type FROM')
        ...
        self.treev_results_vscrb = ttk.Scrollbar(self.search_results, orient ="vertical", command = self.treev_results.yview)    
        ...
        self.add_from_search = Button(self.search_results, image=addtodeckbutton, command = self.add_selected_from_results, borderwidth=0)
        self.add_from_search.place(x = 25, y = 360)
        self.sb_from_search = Button(self.search_results, image=movetosideboardbutton, command = self.sb_selected_from_results, borderwidth=0)
    ...
    def build_main_window(self,window):
        ...
        self.remove_from_deck = Button(root, image=removeselectedbutton, command = self.remove_selected_from_main, borderwidth=0)
        ...
        self.remove_from_sideboard = Button(root, image=removeselectedbutton, command = self.remove_selected_from_sb, borderwidth=0)
            ...
            if self.filter_name.get().lower() in items[0].lower():
                if '----' not in items[0]:
                    ...
                    if 'Creature' in items[2]:
                        self.treev_results.insert("Crt", 'end', text ="L"+str(search_result_counter),values =(items[0], items[1], items[2]))    
                    elif 'Instant' in items[2]:
                        self.treev_results.insert("Inst", 'end', text ="L"+str(search_result_counter),values =(items[0], items[1], items[2]))    
                    elif 'Sorcery' in items[2]:
                        self.treev_results.insert("Sorc", 'end', text ="L"+str(search_result_counter),values =(items[0], items[1], items[2]))    
                    elif 'Artifact' in items[2]:
                        self.treev_results.insert("Arts", 'end', text ="L"+str(search_result_counter),values =(items[0], items[1], items[2]))    
                    elif 'Enchantment' in items[2]:
                        self.treev_results.insert("Ench", 'end', text ="L"+str(search_result_counter),values =(items[0], items[1], items[2]))
                    elif 'Planeswalker' in items[2]:
                        self.treev_results.insert("Plane", 'end', text ="L"+str(search_result_counter),values =(items[0], items[1], items[2]))                    
                    elif 'Land' in items[2]:
                        self.treev_results.insert("Lnd", 'end', text ="L"+str(search_result_counter),values =(items[0], items[1], items[2]))    
                    else:
                        self.treev_results.insert("", 'end', text ="L"+str(search_result_counter),values =(items[0], items[1],items[2]))
    ...
    def show_pie_chart(self):
        ...
        # Plot pie chart
        try:
            plt.pie(pie_sizes, colors=pie_colors, labels = pie_labels, normalize=False, shadow=True, startangle=90, wedgeprops={'linewidth': 2, 'linestyle': 'solid', 'antialiased': True})
        except:
            plt.pie(pie_sizes, colors=pie_colors, labels = pie_labels, shadow=True, startangle=90, wedgeprops={'linewidth': 2, 'linestyle': 'solid', 'antialiased': True})
    ...
    def count_cards(self):
        ...
        return (creature_count, instant_count, sorcery_count, artifact_count, enchantment_count, planeswalker_count, lands_count,
                sb_creature_count, sb_instant_count, sb_sorcery_count, sb_artifact_count, sb_enchantment_count, sb_planeswalker_count, sb_lands_count)
    ...
    def sb_selected_from_results(self):
        ...
        for record in x:            
            ...
            if 'Creature' in item_text[2]:
                self.treev_sideboard.insert("Crt", 'end', text ="L"+str(main_listcounter),values =(item_text[0], item_text[2], 1))
            elif 'Instant' in item_text[2]:
                self.treev_sideboard.insert("Inst", 'end', text ="L"+str(main_listcounter),values =(item_text[0], item_text[2], 1))
            elif 'Sorcery' in item_text[2]:
                self.treev_sideboard.insert("Sorc", 'end', text ="L"+str(main_listcounter),values =(item_text[0], item_text[2], 1))
            elif 'Artifact' in item_text[2]:
                self.treev_sideboard.insert("Arts", 'end', text ="L"+str(main_listcounter),values =(item_text[0], item_text[2], 1))
            elif 'Enchantment' in item_text[2]:
                self.treev_sideboard.insert("Ench", 'end', text ="L"+str(main_listcounter),values =(item_text[0], item_text[2], 1))
            elif 'Planeswalker' in item_text[2]:
                self.treev_sideboard.insert("Plane", 'end', text ="L"+str(main_listcounter),values =(item_text[0], item_text[2], 1))    
            elif 'Land' in item_text[2]:
                self.treev_sideboard.insert("Lnd", 'end', text ="L"+str(main_listcounter),values =(item_text[0], item_text[2], 1))
            else:
                self.treev_sideboard.insert('', 'end', text ="L"+str(main_listcounter),values =(item_text[0], item_text[2], 1))
        ...
        for record in x:
            ...
            if found_flag == False:
                if "-----" not in item_text[0]:
                    # Now to figure out where to put that entry, then see if it is already in there
                    if 'Creature' in item_text[2]:
                        self.treev_maindeck.insert("Crt", 'end', text ="L"+str(main_listcounter),values =(item_text[0], item_text[2], 1))
                    elif 'Instant' in item_text[2]:
                        self.treev_maindeck.insert("Inst", 'end', text ="L"+str(main_listcounter),values =(item_text[0], item_text[2], 1))
                    elif 'Sorcery' in item_text[2]:
                        self.treev_maindeck.insert("Sorc", 'end', text ="L"+str(main_listcounter),values =(item_text[0], item_text[2], 1))
                    elif 'Artifact' in item_text[2]:
                        self.treev_maindeck.insert("Arts", 'end', text ="L"+str(main_listcounter),values =(item_text[0], item_text[2], 1))
                    elif 'Enchantment' in item_text[2]:
                        self.treev_maindeck.insert("Ench", 'end', text ="L"+str(main_listcounter),values =(item_text[0], item_text[2], 1))
                    elif 'Planeswalker' in item_text[2]:
                        self.treev_maindeck.insert("Plane", 'end', text ="L"+str(main_listcounter),values =(item_text[0], item_text[2], 1))    
                    elif 'Land' in item_text[2]:
                        self.treev_maindeck.insert("Lnd", 'end', text ="L"+str(main_listcounter),values =(item_text[0], item_text[2], 1))
                    else:
                        self.treev_maindeck.insert('', 'end', text ="L"+str(main_listcounter),values =(item_text[0], item_text[2], 1))
        ...
        for record in x:
            ...
            if '----' in item_text[0]:
                ...
                if 'Creature' in item_text[1]:
                    self.treev_maindeck.insert("Crt", 'end', text ="L"+str(main_listcounter),values =(item_text[0], item_text[1], item_text[2]))    
                elif 'Instant' in item_text[1]:
                    self.treev_maindeck.insert("Inst", 'end', text ="L"+str(main_listcounter),values =(item_text[0], item_text[1], item_text[2]))    
                elif 'Sorcery' in item_text[1]:
                    self.treev_maindeck.insert("Sorc", 'end', text ="L"+str(main_listcounter),values =(item_text[0], item_text[1], item_text[2]))    
                elif 'Artifact' in item_text[1]:
                    self.treev_maindeck.insert("Arts", 'end', text ="L"+str(main_listcounter),values =(item_text[0], item_text[1], item_text[2]))    
                elif 'Enchantment' in item_text[1]:
                    self.treev_maindeck.insert("Ench", 'end', text ="L"+str(main_listcounter),values =(item_text[0], item_text[1], item_text[2]))
                elif 'Planeswalker' in item_text[1]:
                    self.treev_maindeck.insert("Plane", 'end', text ="L"+str(main_listcounter),values =(item_text[0], item_text[1], item_text[2]))                    
                elif 'Land' in item_text[1]:
                    self.treev_maindeck.insert("Lnd", 'end', text ="L"+str(main_listcounter),values =(item_text[0], item_text[1], item_text[2]))    
                else:
                    self.treev_maindeck.insert('', 'end', text ="L"+str(main_listcounter),values =(item_text[0], item_text[1], item_text[2]))    
        ...
        for record in x:
            ...
            if '----' in item_text[0]:
                ...
                if 'Creature' in item_text[1]:  #  An Enchanted or Artifact Creature will be classified as a creature
                    self.treev_sideboard.insert("Crt", 'end', text ="L"+str(side_listcounter),values =(item_text[0], item_text[1], item_text[2]))    
                elif 'Instant' in item_text[1]:
                    self.treev_sideboard.insert("Inst", 'end', text ="L"+str(side_listcounter),values =(item_text[0], item_text[1], item_text[2]))    
                elif 'Sorcery' in item_text[1]:
                    self.treev_sideboard.insert("Sorc", 'end', text ="L"+str(side_listcounter),values =(item_text[0], item_text[1], item_text[2]))    
                elif 'Artifact' in item_text[1]:
                    self.treev_sideboard.insert("Arts", 'end', text ="L"+str(side_listcounter),values =(item_text[0], item_text[1], item_text[2]))    
                elif 'Enchantment' in item_text[1]:
                    self.treev_sideboard.insert("Ench", 'end', text ="L"+str(side_listcounter),values =(item_text[0], item_text[1], item_text[2]))
                elif 'Planeswalker' in item_text[1]:
                    self.treev_sideboard.insert("Plane", 'end', text ="L"+str(side_listcounter),values =(item_text[0], item_text[1], item_text[2]))                    
                elif 'Land' in item_text[1]:
                    self.treev_sideboard.insert("Lnd", 'end', text ="L"+str(side_listcounter),values =(item_text[0], item_text[1], item_text[2]))    
                else:
                    self.treev_sideboard.insert('', 'end', text ="L"+str(side_listcounter),values =(item_text[0], item_text[1], item_text[2]))
    ...
    def save_deck(self):
        ...
        if len(self.deck_table) > 0:
            ...
            for child in children:
                ...
                if "----" not in childname:    import_into_deck(self.deck_table, childname, childtype, int(childqty), "-")
            ...
            for child in children:
                ...
                if "----" not in childname: import_into_deck(self.deck_table, childname, childtype, int(childqty), "Yes")        
        ...
        print ("saving deck..ID ",self.deck_id, "name: ",self.deck_name,"commander: ", self.commander,"partner: ", self.partner,"tier: ", self.current_tier,"table; ", self.deck_table,"status: ", current_deckstatus,"location: ",
               self.decklocation,"notes: ", self.deck_notes)
        
        print ("partner type: ", type(self.partner), "   tier type: ", type(self.deck_tier))
        create_new_deck(self.deck_id, self.deck_name, self.commander, self.partner, self.deck_tier, self.deck_table, current_deckstatus, self.decklocation, self.deck_notes)
        self.close_deck()
    ...
    def prep_deck_list(self):
        ...
        self.treev_maindeck.insert("", 1, text ="Crt", iid ="Crt", open=True, values = ("------CREATURES------","------------","------"))
        self.treev_maindeck.insert("", 2, text ="Inst",iid ="Inst", open=True, values = ("------INSTANTS------","------------","------"))
        self.treev_maindeck.insert("", 3, text ="Sorc", iid ="Sorc", open=True, values = ("------SORCERIES------","------------","------"))
        self.treev_maindeck.insert("", 4, text ="Arts",iid ="Arts", open=True, values = ("------ARTIFACTS------","------------","------"))
        self.treev_maindeck.insert("", 5, text ="Ench",iid ="Ench", open=True, values = ("------ENCHANTMENTS------","------------","------"))
        self.treev_maindeck.insert("", 6, text ="Plane",iid ="Plane", open=True, values = ("------PLANESWALKERS------","------------","------"))
        self.treev_maindeck.insert("", 7, text ="Lnd", iid ="Lnd", open=True, values = ("------LANDS------","------------","------"))
        
        self.treev_sideboard.insert("", 1, text ="Crt", iid ="Crt", open=True, values = ("------CREATURES------","------------","------"))
        self.treev_sideboard.insert("", 2, text ="Inst", iid ="Inst", open=True, values = ("------INSTANTS------","------------","------"))
        self.treev_sideboard.insert("", 3, text ="Sorc", iid ="Sorc", open=True, values = ("------SORCERIES------","------------","------"))
        self.treev_sideboard.insert("", 4, text ="Arts", iid ="Arts", open=True, values =  ("------ARTIFACTS------","------------","------"))
        self.treev_sideboard.insert("", 5, text ="Ench", iid ="Ench", open=True, values =  ("------ENCHANTMENTS------","------------","------"))
        self.treev_sideboard.insert("", 6, text ="Plane", iid ="Plane", open=True, values = ("------PLANESWALKERS------","------------","------"))
        self.treev_sideboard.insert("", 7, text ="Lnd", iid ="Lnd", open=True, values = ("------LANDS------","------------","------"))        
    ...
    def prep_search_result_list(self):
        ...
        self.treev_results.insert("", 1, text ="Crt", iid ="Crt", open=True, values = ("------CREATURES------","------------","-----------"))
        self.treev_results.insert("", 2, text ="Inst", iid ="Inst", open=True, values = ("------INSTANTS------","------------","------------"))
        self.treev_results.insert("", 3, text ="Sorc", iid ="Sorc", open=True, values = ("------SORCERIES------","------------","------------"))
        self.treev_results.insert("", 4, text ="Arts", iid ="Arts", open=True, values =  ("------ARTIFACTS------","------------","------------"))
        self.treev_results.insert("", 5, text ="Ench", iid ="Ench", open=True, values =  ("------ENCHANTMENTS------","------------","------------"))
        self.treev_results.insert("", 6, text ="Plane", iid ="Plane", open=True, values = ("------PLANESWALKERS------","------------","------------"))
        self.treev_results.insert("", 7, text ="Lnd", iid ="Lnd", open=True, values = ("------LANDS------","------------","------------"))        
    ...
    def load_deck(self, deck):
        ...
        for items in lst:
            if items[3]=='Yes':
                ...
                if 'Creature' in items[1]:
                    self.treev_sideboard.insert("Crt", 'end', text ="L"+str(side_listcounter),values =(items[0], items[1], items[2]))    
                elif 'Instant' in items[1]:
                    self.treev_sideboard.insert("Inst", 'end', text ="L"+str(side_listcounter),values =(items[0], items[1], items[2]))    
                elif 'Sorcery' in items[1]:
                    self.treev_sideboard.insert("Sorc", 'end', text ="L"+str(side_listcounter),values =(items[0], items[1], items[2]))    
                elif 'Artifact' in items[1]:
                    self.treev_sideboard.insert("Arts", 'end', text ="L"+str(side_listcounter),values =(items[0], items[1], items[2]))    
                elif 'Enchantment' in items[1]:
                    self.treev_sideboard.insert("Ench", 'end', text ="L"+str(side_listcounter),values =(items[0], items[1], items[2]))
                elif 'Planeswalker' in items[1]:
                    self.treev_sideboard.insert("Plane", 'end', text ="L"+str(side_listcounter),values =(items[0], items[1], items[2]))                    
                elif 'Land' in items[1]:
                    self.treev_sideboard.insert("Lnd", 'end', text ="L"+str(side_listcounter),values =(items[0], items[1], items[2]))    
                else:
                    self.treev_sideboard.insert("", 'end', text ="L"+str(side_listcounter),values =(items[0], items[1], items[2]))
            else:
                main_listcounter +=1
                if 'Creature' in items[1]:
                    self.treev_maindeck.insert("Crt", 'end', text ="L"+str(main_listcounter),values =(items[0], items[1], items[2]))
                elif 'Instant' in items[1]:
                    self.treev_maindeck.insert("Inst", 'end', text ="L"+str(side_listcounter),values =(items[0], items[1], items[2]))    
                elif 'Sorcery' in items[1]:
                    self.treev_maindeck.insert("Sorc", 'end', text ="L"+str(side_listcounter),values =(items[0], items[1], items[2]))    
                elif 'Artifact' in items[1]:
                    self.treev_maindeck.insert("Arts", 'end', text ="L"+str(side_listcounter),values =(items[0], items[1], items[2]))    
                elif 'Enchantment' in items[1]:
                    self.treev_maindeck.insert("Ench", 'end', text ="L"+str(side_listcounter),values =(items[0], items[1], items[2]))    
                elif 'Land' in items[1]:
                    self.treev_maindeck.insert("Lnd", 'end', text ="L"+str(side_listcounter),values =(items[0], items[1], items[2]))    
                elif 'Planeswalker' in items[1]:
                    self.treev_maindeck.insert("Plane", 'end', text ="L"+str(side_listcounter),values =(items[0], items[1], items[2]))
                else:
        ...
        if len(self.partner) >0 :
            ...
            mtg_sql.cur.execute("SELECT Mana_cost, Image_URL, Colour_ID FROM "+this_table+" WHERE Name=?",search_partner)
            fetch = mtg_sql.cur.fetchone()
            partner_img = fetch[1]
            self.partner_cmc_label.configure(text = 'Partner Mana Cost: '+fetch[0])
            self.partner_label.configure(text = 'Partner: '+self.partner)
