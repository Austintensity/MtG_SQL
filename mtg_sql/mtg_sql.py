...
if type(file_to_open) is str:
    db = sqlite3.connect(file_to_open)
else:
    db = sqlite3.connect(str(file_to_open))
...
def insert_card_from_json(name, colors, colorid, manacost, cmc, c_type, oracletext, keywords, power, toughness, 
                          loyalty, rarity, image):
    # sqlite_insert_with_param = """INSERT OR IGNORE INTO Grandtable (Name, Colors, Colour_ID, Mana_Cost, 
    # CMC, Card_Type, Oracle_Text, Keywords, Power, Toughness, Loyalty, Rarity, Image_URIs) VALUES (?, ?, ?, ?, ?, 
    # ?, ?, ?, ?, ?, ?, ?, ?)"""
    
    sqlite_insert_with_param = """INSERT OR IGNORE INTO GrandtableCondensed (Name, Colors, Colour_ID, Mana_Cost, CMC, 
    Card_Type, Oracle_Text, Keywords, Power, Toughness, Loyalty, Rarity, Image_URIs) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?,
    ?, ?, ?, ?) """
    
    add_tuple_to_sql = (name, colors, colorid, manacost, cmc, c_type, oracletext, keywords, power, toughness, loyalty, 
                        rarity, image)
...
def insert_splitcard_from_json(name1, name2, layout, colors, colorid, manacost, cmc, c_type, oracletext, keywords, 
                               power, toughness, loyalty, rarity, image):
    # sqlite_insert_with_param1 = """INSERT OR IGNORE INTO GrandtableSplit (Face_Name, Flip_Name, Layout, Colors, 
    # Colour_ID, Mana_Cost, CMC, Type_Line, Oracle_Text, Keywords, Power, Toughness, Loyalty, Rarity, Image_URIs) 
    # VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"""
    
    sqlite_insert_with_param = """INSERT OR IGNORE INTO GrandtableSplitCondensed (Face_Name, Flip_Name, Layout, Colors, 
    Colour_ID, Mana_Cost, CMC, Type_Line, Oracle_Text, Keywords, Power, Toughness, Loyalty, Rarity, Image_URIs) VALUES 
    (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"""

    add_tuple_to_sql = (name1, name2, layout, colors, colorid, manacost, cmc, c_type, oracletext, keywords, power, 
                        toughness, loyalty, rarity, image)
    # print (sqlite_insert_with_param, add_tuple_to_sql)
    with db:
...
def insert_card(name, clr_id, mana_cost, cmc, c_type, keywords, is_legend, has_etb, has_ramp, has_draw, has_target, 
                has_wipe, has_activated, has_triggered, image_url, oracle_text):
    sqlite_insert_with_param = """INSERT OR IGNORE INTO Cards_Condensed (Name, Colour_ID, Mana_Cost, CMC, Card_Type, 
    Keywords, Legendary, ETB, Ramp, Draw, Target, Broad, Activated, Triggered, Image_URL, Oracle_Text) VALUES (?, ?, 
    ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"""
    
    # sqlite_insert_with_param1 = """INSERT OR IGNORE INTO Cards (Name, Colour_ID, Mana_Cost, CMC, Card_Type, Keywords, 
    # Legendary, ETB, Ramp, Draw, Target, Broad, Activated, Triggered, Image_URL, Oracle_Text) VALUES (?, ?, ?, ?, ?, 
    # ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""" 
    add_tuple_to_sql = (name, str(clr_id), str(mana_cost), int(cmc), c_type, str(keywords), str(is_legend), 
                        str(has_etb), str(has_ramp), str(has_draw), str(has_target), str(has_wipe), str(has_activated), 
                        str(has_triggered), image_url, oracle_text)
...
def insert_face_card(name, layout, flipname, clr_id, mana_cost, cmc, c_type, keywords, is_legend, has_etb, has_ramp, 
                     has_draw, has_target, has_wipe, has_activated, has_triggered, image_url, oracle_text):
    sqlite_insert_with_param = """INSERT OR IGNORE INTO Face_Cards_Condensed (Name, Layout, Flip_Name, Colour_ID, 
    Mana_Cost, CMC, Card_Type, Keywords, Legendary, ETB, Ramp, Draw, Target, Broad, Activated, Triggered, Image_url, 
    Oracle_Text) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"""
    
    # sqlite_insert_with_param1 = """INSERT OR IGNORE INTO Face_Cards (Name, Layout, Flip_Name, Colour_ID, Mana_Cost, 
    # CMC, Card_Type, Keywords, Legendary, ETB, Ramp, Draw, Target, Broad, Activated, Triggered, Image_url, 
    # Oracle_Text) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"""

    add_tuple_to_sql = (name, layout, flipname, str(clr_id), str(mana_cost), int(cmc), c_type, str(keywords), 
                        is_legend, str(has_etb), str(has_ramp), str(has_draw), str(has_target), str(has_wipe), 
                        str(has_activated), str(has_triggered), image_url, oracle_text)
...
def insert_land(name, clr_id, mana, mana_clr, act_abil):
    sqlite_insert_with_param = "INSERT OR IGNORE INTO Lands (Name, Colour_ID, Max_Mana, Mana_Clrs, " \
                               "Activated_Abilities) VALUES (?, ?, ?, ?, ?)"
...
def insert_creature(name, cr_type, cmc, power, toughness, keywords):
    sqlite_insert_with_param = "INSERT OR IGNORE INTO Creatures (Name, Creature_Type, CMC, Power, Toughness, " \
                               "Keywords) VALUES (?, ?, ?, ?, ?, ?)"
...
def insert_planeswalker(name, loyalty, is_passive, boost_ability, cost_ability, ultimate):
    sqlite_insert_with_param = """INSERT OR IGNORE INTO Planeswalkers (Name, Loyalty, Is_Passive, Boost_Ability, 
    Cost_Ability, Ultimate)
            VALUES (?, ?, ?, ?, ?, ?)"""
...
def insert_deck(d_id, name, clr_id, commander, partner, tier, decklist, avg_cmc, lands, ramp, card_draw, 
                target_removal, board_wipe, etb, status, location, notes):
    sqlite_insert_with_param = """INSERT OR IGNORE INTO All_Decks (id_Code, Deck_Name, Colour_ID, Commander, Partner, 
    Deck_Tier, Deck_list, Avg_CMC, Lands, Ramp, Draw, Target_Removal, Board_Wipe, ETB_Effects, Status, Location, Notes) 
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"""
    add_tuple_to_sql = (d_id, name, str(clr_id), commander, partner, str(tier), decklist, avg_cmc, lands, ramp,
                        card_draw, target_removal, board_wipe, etb, status, location, notes)
    with db:
        cur.execute(sqlite_insert_with_param, add_tuple_to_sql)


def insert_game(gm_id, playerlist, winner, loser, turns, notes):
    p1 = ""
    p2 = ""
    p3 = ""
    p4 = ""
    ...
    with db:
        ...
        #             """, {'gm_id': gm_id,'winner': winner, 'loser': loser, 'turns': turns, 'notes': notes, 'p1': p1,
        #             'p2': p2, 'p3': p3, 'p4': p4, 'p5': p5,'p6': p6})
        cur.execute("""INSERT INTO Test_Matches VALUES (
                   :gm_ID,:p1,:p2,:p3,:p4,:p5,:p6,:winner,:loser,:turns,:notes)
                    """, {'gm_ID': gm_id, 'p1': p1, 'p2': p2, 'p3': p3, 'p4': p4, 'p5': p5, 'p6': p6, 'winner': winner,
                          'loser': loser, 'turns': turns, 'notes': notes})

        count = 1
        for players in playerlist:
            sqlstatement = """UPDATE Test_Matches SET Deck_""" + str(count) + """ = """ + '"' + players + \
                           '"' + """ WHERE gm_ID = """ + str(gm_id)
            print(sqlstatement)
...
#         recreate table and populate
        guilds = [MtgColours('R', 'MONO RED'),
                  MtgColours('U', 'MONO BLUE'),
                  MtgColours('W', 'MONO WHITE'),
                  MtgColours('G', 'MONO GREEN'),
                  MtgColours('B', 'MONO BLACK'),
                  MtgColours('C', 'COLOURLESS'),
                  MtgColours('BR', 'RAKDOS'),
                  MtgColours('BG', 'GOLGARI'),
                  MtgColours('BU', 'DIMIR'),
                  MtgColours('BW', 'ORZHOV'),
                  MtgColours('WU', 'AZORIUS'),
                  MtgColours('WR', 'BOROS'),
                  MtgColours('WG', 'SELESNYA'),
                  MtgColours('UG', 'SIMIC'),
                  MtgColours('RG', 'GRUUL'),
                  MtgColours('UR', 'IZZET'),
                  MtgColours('WUB', 'ESPER'),
                  MtgColours('WUR', 'JESKAI'),
                  MtgColours('WUG', 'BANT'),
                  MtgColours('URB', 'GRIXIS'),
                  MtgColours('URG', 'SULTAI'),
                  MtgColours('BRG', 'JUND'),
                  MtgColours('WRG', 'NAYA'),
                  MtgColours('UGR', 'TEMUR'),
                  MtgColours('WRB', 'MARDU'),
                  MtgColours('WBG', 'ABZAN'),
                  MtgColours('WURG', 'INK TREADER'),
                  MtgColours('WUBR', 'YORE TILLER'),
                  MtgColours('URBG', 'GLINT EYE'),
                  MtgColours('WRBG', 'DUNE BROOD'),
                  MtgColours('WUBG', 'WITCH MAW'),
                  MtgColours('WUBRG', 'WOO BERG')
                  ]
...
"""### MANAGING DECK TABLES ###  START ###
    ...
    1. Running import_deck_fromtxt() to import a deck list (exported from deckbox.org, etc) and give a name to that 
    table, which will be referred to as deck_list 
...
def check_cardref(card_name):  # check under which table a particular card needs to be looked up
    ...
    if ref_table is None:
        """Sometimes this search is done with double faced cards but only part of the card name was searched and it 
        wasn't found exactly, so searching again will check for those"""
        card_name = card_name + " //%"
        this = "%"+card_name
        search_this = (this,)
...
def handle_five_things(sql_cursor, qty, current_wp, current_dr, current_rmp, current_tg, current_etb, current_land, 
                       current_cmc):
    """it's called fivethings, even though it is up to 7, I think for face cards, it will even be 8.  Tomorrow, 
    we'll see..
    Name of method may change or might not
    Called by calculate_deck() method, this will review the fetch data from the sql_cursor and increment the fields, 
    and provide new updated counts  """
    (new_wipe, new_draw, new_ramp, new_target, new_etb, new_land, new_cmc) = (current_wp, current_dr, current_rmp, 
                                                                              current_tg, current_etb, current_land, 
                                                                              current_cmc)
    
    if sql_cursor is not None:
        (cmc, wipe, draw, ramp, target, etb, card_type) = (sql_cursor[0], sql_cursor[1], sql_cursor[2],
                                                           sql_cursor[3], sql_cursor[4], sql_cursor[5],
                                                           sql_cursor[6])
        if wipe != '-':
        ...
        if 'land' in card_type.lower():
            new_land = current_land + qty
...
def calculate_deck(deck_list):
    ...
    returns: count of all board wipes, draw cards/engines, ramp cards, targeting cards, etb cards, lands, and average 
    converted mana cost, or 'mana value')     """
    print(deck_list)
    wipe_count = 0
    draw_count = 0
    ramp_count = 0
        ...
        if search_this[1] == 'Yes':
            ...
            if this_table is not None:
                if "Face_Cards_Condensed" in this_table:   
                    ...
                    # Convert 'Akki Lavarunner // Tok-Tok, Volcano Born' to just 'Akki Lavarunner' since the card
                    # data is represented as 2 rows
                    # First search for 'Akki Lavarunner', and then search for 'Tok-Tok, Volcano Born'
                    # (which will be the next row)
                    cur.execute("""SELECT CMC, Broad, Draw, Ramp, Target, ETB, Card_Type, Flip_Name FROM 
                    Face_Cards_Condensed WHERE Name=?""", this_name)
                    fivethings = cur.fetchone()
                    (new_wipe, new_draw, new_ramp, new_target, new_etb, new_land, new_cmc) = \
                        handle_five_things(fivethings, score_multiplier, wipe_count, draw_count, ramp_count,
                                           target_count, etb_count, land_count, cmc_total)

                    if fivethings[6] is not None:
                        if 'land' in fivethings[6].lower():
                            double_faced_land_flag = True
                        
                    this_name = (fivethings[7],)
                    cur.execute("""SELECT CMC, Broad, Draw, Ramp, Target, ETB, Card_Type FROM Face_Cards_Condensed 
                    WHERE Name=?""", this_name)
                    # Running this SELECT again so it counts the stats on both cards.. this will incidentally cause
                    # some strange average cmcs if hypothetically a deck had several copies of a modal_dfc card
                    fivethings = cur.fetchone()            
                    wipe_count, draw_count, ramp_count, target_count, etb_count, land_count, cmc_total = \
                        handle_five_things(fivethings, score_multiplier, new_wipe, new_draw, new_ramp, new_target,
                                           new_etb, new_land, new_cmc)
                    # print(name, ": after this, current target count: " , target_count)
                    if fivethings[6] is not None:
                        if 'land' in fivethings[6].lower() and double_faced_land_flag is True:
                            land_count -= 1
                else:
                    # Get card data for single sided card and if it has
                    this_name = (name,)
                    cur.execute("""Select CMC, Broad, Draw, Ramp, Target, ETB, Card_Type FROM Cards_Condensed WHERE 
                    Name=?""", this_name)
                    fivethings = cur.fetchone()            
                    wipe_count, draw_count, ramp_count, target_count, etb_count, land_count, cmc_total = \
                        handle_five_things(fivethings, score_multiplier, wipe_count, draw_count, ramp_count,
...
def import_deck_fromtxt(textfile, deck_list):
    ...
    Parameters: 'textfile': pretty self explanatory
                'deck_list': Give a name for the SQLite table for the deck, starting with the letter 'z' for
                alphabetical ease """
...
def create_new_deck(id_code, deck_name, commander, partner, tier, deck_list, status, location, notes):
    """Adding to the the All_Decks SQL table by running insert_deck().  RUNS calculate_deck(), then insert_deck()
    Returns nothing.
        Parameters: deck_name: The official name of the deck 'Smithers, who is this gastropod?' (this is one of the few
        decks I named that doesn't name the commander)
                    ...
                    tier: Referring to primary- one each of the 32 different color combinations, or
                                        ...
                                        or tertiary- because why not <Zoidberg>? maybe for playtests, or casual or
                                        playing against a niece or something
                    deck_list: The specific table name for the deck imported from the text file. Should start with a
                    lowercase 'z' so they are at the bottom of the table list alphabetically
                    status: Active status of the deck {'ACTIVE', 'INACTIVE' (unassembled),'HARVESTED'(partially
                    assembled)}
                    id_code:  Manually** created based on the commander's colour identity then follows the following
                    fun little system (not alphabetical).
    ...
    if len(partner) > 0:
        print("Partner Commander: ", partner)
    ...
    print("insert_deck start..", deck_name)
    insert_deck(id_code, deck_name, clr_id, commander, partner, tier, deck_list, avg_cmc, land_count, ramp_count,
                draw_count, target_count, wipe_count, etb_count, status, location, notes)
    print()
...
def delete_all_zdecktables():
    ...
    results = cur.fetchall()
    for deck_name in results:
        cur.execute("DROP TABLE IF EXISTS '"+deck_name[0]+"'")


"""### TEMPORARY DECK TABLES   ###
  (preselected group of already downloaded txt files in decks folder)"""
...
def import_all_decks():
    ...
    import_deck_fromtxt('R-AT04_Magda & (7 dwarves) Dorky, Broody, Horny, Loopy, Creepy, Twitchy, and '
                        'Bill.txt', 'zMagda')
...
def create_all_decks():
    ...
    create_new_deck('W-TW02', "Odric Keyword Smithy", 'Odric, Lunarch Marshal', "", "Primary", "zOdric",
                    'ACTIVE', 'SINGLE WHITE BOX', '')
    create_new_deck('U-c&ok03', "M'Orvar vor me", "Orvar, the All-Form", "", "Primary", "zOrvar", 'ACTIVE',
                    'SINGLE BLUE BOX', '')
    create_new_deck('B-SG01', "Shirei's Happy Sack", "Shirei, Shizo's Caretaker", "", "Primary", "zShirei",
                    'ACTIVE', 'SINGLE BLACK BOX', '')
    create_new_deck('R-AT04', "Magda & (7 dwarves) Dorky, Broody, Horny, Loopy, Creepy, Twitchy, and Bill",
                    'Magda, Brazen Outlaw', "", "Primary", "zMagda", 'ACTIVE', 'SINGLE RED BOX', '')
    create_new_deck('G-Id03', "Fynn for fun cuz Marwyn's no nurturer", 'Fynn, the Fangbearer', "", "Primary",
                    "zFynn", 'ACTIVE', 'SINGLE GREEN BOX', '')
    create_new_deck('UG-KC03', "Verazol's Kick'n it.txt", 'Verazol, the Split Current', "", "Primary",
                    "zVerazol", 'ACTIVE', 'SINGLE TURQUOISE', '')
    create_new_deck('URBG-TG01', "Zombie uprising!", "Kraum, Ludevic's Opus", "Ikra Shidiqi, the Usurper",
                    "Primary", "zKraumIkra", 'ACTIVE', 'SINGLE BLUE BOX-unmarked', '')
    create_new_deck('WBG-Gcy02', "Way-big Juicy Cycling!", 'Nethroi, Apex of Death', "", "Primary", "zNethroi",
                    'ACTIVE', 'FANCY GREEN BOX', '')
    # BORN OF THE GODS
    create_new_deck('BR-01', "Bottom's up, Grenzo!", 'Grenzo, Dungeon Warden', "", "Primary", "zBottoms_Up_Grenzo",
                    'ACTIVE', 'BORN OF THE GODS', '')
    create_new_deck('WUBG-C01', "Attraxa Praetors' +1 Sword", "Atraxa, Praetors' Voice", "", "Primary", "zAttraxa",
                    'ACTIVE', 'BORN OF THE GODS', '')
    create_new_deck('WBR-T01', 'Hold the garlic', 'Edgar Markov', "", "Primary", "zEdgar_Markov", 'ACTIVE',
                    'BORN OF THE GODS', '')
    # DRAGONS MAZE
    create_new_deck('WUR-T04', 'Kykar Spirit Splicer', "Kykar, Wind's Fury", "", 'Primary', 'zKykar', 'ACTIVE',
                    'DRAGONS MAZE', '')
    create_new_deck('RG-$U03', "Alenalana Buyback X-Closer", 'Alena, Kessig Trapper', "Halana, Kessig Ranger",
                    "Primary", "zAlena_Halana", 'ACTIVE', 'DRAGONS MAZE', '')
    create_new_deck('UR-D01', "It is a Niv Mizet visit, isn't it?  it is!", 'Niv-Mizzet, Parun', "", "Primary",
                    "zNivMizzet", 'ACTIVE', 'DRAGONS MAZE', '')
    # PLAIN BLUE
    create_new_deck('WUB-Ut02', "Freaky Merieke!", 'Merieke Ri Berit', "", "Primary", "zMerieke", 'ACTIVE',
                    'PLAIN BLUE', '')
    create_new_deck('UB-n01', "Yuriko's Ninjitsu", "Yuriko, the Tiger's Shadow", "", "Primary", "zYuriko",
                    'ACTIVE', 'PLAIN BLUE', '')
    create_new_deck('UBR-T01', "Inalla wizards ya had to include", 'Inalla, Archmage Ritualist', "", "Primary",
                    "zInalla", 'ACTIVE', 'PLAIN BLUE', '')
    # PLAIN BLACK
    create_new_deck('WUBRG-T01', "Smithers, who is this gastropod?", 'Sliver Overlord', "", "Primary", "zSmithers",
                    'ACTIVE', 'PLAIN BLACK', '')
    create_new_deck('UBG-I02', "Volrath the Infectious", 'Volrath, the Shapestealer', "", "Primary", "zVolrath",
                    'ACTIVE', 'PLAIN BLACK', '')
    create_new_deck('C-A01', "Traxos and his Hope of Gary Cooper", 'Traxos, Scourge of Kroog', "", "Primary",
                    "zTraxos", 'ACTIVE', 'PLAIN BLACK', '')
    # THEROS
    create_new_deck('URG-tS01', "Yasova Stickyclaws", 'Yasova Dragonclaw', "", "Primary", "zYasova", 'ACTIVE',
                    'THEROS', '')
    create_new_deck('BRG-L01', "Lord Windgrace Destructive Flow", 'Lord Windgrace', "", "Primary", "zWindgrace",
                    'ACTIVE', 'THEROS', '')
    create_new_deck('WB-a02', "Teysa Aristrocrats", 'Teysa Karlov', "", "Primary", "zTeysa", 'ACTIVE', 'THEROS', '')
    # HOUR OF DEVASTATION
    create_new_deck('WBRG-BE01', "Saskia Combat Gimmicks", 'Saskia the Unyielding', "", "Primary", "zSaskia",
                    'ACTIVE', 'HOUR OF DEVASTATION', '')
    create_new_deck('WRG-RG03', "Obuun'dn't like me when I'm angry!", 'Obuun, Mul Daya Ancestor', "", "Primary",
                    "zObuun", 'ACTIVE', 'HOUR OF DEVASTATION', '')
    create_new_deck('BG-CTok02', "Hapatra", 'Hapatra, Vizier of Poisons', "", "Primary", "zHapatra", 'ACTIVE',
                    'HOUR OF DEVASTATION', '')
    # IXALAN
    create_new_deck('WG-Pok01', "Trostani Tokens", "Trostani, Selesnya's Voice", "", "Primary", "zTrostani",
                    'ACTIVE', 'IXALAN', '')
    create_new_deck('WU-EXbe01', "Medomai - Timeless", 'Medomai the Ageless', "", "Primary", "zMedomai",
                    'ACTIVE', 'IXALAN', '')
    create_new_deck('WR-01', "Feat her to defeat her", 'Feather, the Redeemed', "", "Primary", "zFeather",
                    'ACTIVE', 'IXALAN', '')
    # ORIGINS
    create_new_deck('WUBR-A02', "Batman & Robin", 'Silas Renn, Seeker Adept', "Akiri, Line-Slinger", "Primary",
                    "zSilasAkiri", 'ACTIVE', 'ORIGINS', '')
    create_new_deck('WURG-H01', "Meletis says 'Hugs not thugs'", "Kynaios and Tiro of Meletis", "", 'Primary',
                    'zMeletis', 'INACTIVE', 'ORIGINS', '')
    create_new_deck('WUG-Lc02', "I Chu-Chu-Chulane you", 'Chulane, Teller of Tales', "", 'Primary', 'zChulane',
                    'ACTIVE', 'ORIGINS', '')

    # SECONDARY DECKS #
    create_new_deck('WBR-G02', "Ashley, the disgraceful Graverobber", 'Alesha, Who Smiles at Death', "", "Secondary",
                    "zAshley", 'ACTIVE', 'RIVALS OF IXALAN', '')
    create_new_deck('WUB-N03', "Leela's Enchanter", 'Alela, Artful Provocateur', "", "Secondary", "zAlela", 'ACTIVE',
                    'RIVALS OF IXALAN', '')
    create_new_deck('BRG-STok03', "I wanna be Korvold, sucka!", 'Korvold, Fae-Cursed King', "", "Secondary",
                    "zKorvold", 'ACTIVE', 'RIVALS OF IXALAN', '')

    # ONLINE DECKS #
    create_new_deck('WU-0X', "No agony, no BRAGOny", 'Brago, King Eternal', "", "Online", "zBrago",
                    'UNASSEMBLED', '-', '')
    create_new_deck('WB-0X', "Tribal Lannister", 'General Kudro of Drannith', "", "Online", "zLannister",
                    'UNASSEMBLED', '-', '')
    create_new_deck('WR-0X', "Koll's affordable dwarvish crap", 'Koll, the Forgemaster', "", "Online",
                    "zKoll", 'UNASSEMBLED', '-', '')
    create_new_deck('G-0X', "Toskivasion", 'Toski, Bearer of Secrets', "", "Online", "zToski",
                    'UNASSEMBLED', '-', '')
...
def main():
    ...
    # cur.execute("VACUUM")
    # Clean up emptied and recycled spaces in the database after several mass deletions  (like defragging)
    
    check_all_tables()
    # SEE IF TABLES EXIST, AND CREATES IF REQUIRED #
    cur.execute("SELECT Cards_Condensed.Name, Cards_Condensed.Mana_Cost, Cards_Condensed.Target, "
                "Cards_Condensed.Broad FROM Cards_Condensed INNER JOIN zAttraxa ON "
                "zAttraxa.Name=Cards_Condensed.Name WHERE length(Cards_Condensed.Target)> 2 or "
                "length(Cards_Condensed.Broad) > 2")
    results = cur.fetchall()
    for result in results:
        print(result)
    # Run this block to rebuild all my decklists from the last exported text files
