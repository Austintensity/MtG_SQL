...
def ETB_Search(oracletext, card_type, keywords):
    ...
    if has_etb == 'TRUE' :
        etb = regex_get_ETB(oracletext, card_type, keywords)
    else:
        etb = "-"
    return etb
...
def Draw_Search(oracletext):
    ...
    if 'draw' in oracletext:
        has_draw = regex_get_draw(oracletext)
    else:
        has_draw = '-'
    return has_draw
...
def Target_Search(oracletext, card_type, keywords):
    ...
    **This was partially on the MTG_Regex page under regex_get_target() but I was having issues so it was all combined and moved here
    ...
    if 'target' in oracletext.lower():
        ...
        pattern = re.compile(regex_searchmode['target_filter'])    
        ...
        if re.findall(regex_etb_modes['mill'], oracletext, re.IGNORECASE): target = target + "Mill "
        
        for filters in regex_target_modes:
            # print (re.findall(regex_target_modes[filters], oracletext, re.IGNORECASE))
            if re.findall(regex_target_modes[filters], oracletext, re.IGNORECASE):
                # print ("found ", filters)
...
def Ramp_Search(oracletext, card_type):
    ...
    *Lands don't count as ramp cards
    ...
    if has_ramp == 'TRUE':
        ramp_abilities = regex_get_ramp(oracletext, card_type)
    else:
        ramp_abilities = '-' 
...
def Wipe_Search(oracletext, keywords):
    ...
    if has_wipe == 'TRUE' : 
        has_wipe = regex_get_wipe(oracletext, keywords)
        # print (has_wipe)
...
def Trigger_Search(oracletext, keywords):
    ...
    for terms in triggered_ability_terms:
        if terms in oracletext or terms in oracletext.lower() : triggered_ability = regex_get_triggered(oracletext, keywords)
...
def Activated_Search(oracletext, keywords):  
    ...
    for terms in activated_ability_terms:
        if terms in oracletext or terms in oracletext.lower() : activated_ability = regex_get_activated(oracletext, keywords)
...
def Land_handle(name, oracletext, clr_ID):
    ...
    """
    act_abilities = regex_get_aac(oracletext)
    colors = []
    colors = regex_get_mclr(oracletext)
    if colors is None: colors = {'C'}

    for findmodes in regex_searchmode:
        if 'mana' in findmodes:
            found_mana = regex_get_mv(regex_searchmode[findmodes],oracletext)
    ...
    """
    if x_found == 'TRUE':
        insert_land(name, clr_ID, 'X', str(colors), act_abilities)
    else:
        insert_land(name, clr_ID, greatest_mana, str(colors), act_abilities)

def Spell_handle(name, oracletext, spelltype):   # REGEX is doing all the heavy lifting on this one
    heal_ability = regex_get_lifegains(oracletext)
    damage_ability = regex_get_dmggains(oracletext)
    insert_spell(name, spelltype, heal_ability, damage_ability)
...
def Planeswalker_handle(name, oracletext, loyalty):
    ...
    count_costs = 0
    high_cost_ability = 0
    get_boost = regex_get_pboosts(oracletext)
    ...
    get_cost_ability = regex_get_pcosts(oracletext)
    ...
    if 'can be your commander' in oracletext.lower():
        ...
        is_passive = regex_get_ppa(oracletext)
    ...
    if boost_count == 0:
        if ultimate_ability == 'TRUE' :
            if count_costs == 1:
                insert_planeswalker(name, loyalty, is_passive, "-", cost_ability, high_cost_ability)
            elif count_costs == 2:
                insert_planeswalker(name, loyalty, is_passive, "-", cost_ability, "-")
        else: 
            insert_planeswalker(name, loyalty, is_passive, "-", cost_ability, "-")
    elif boost_count == 1:
        ...
        if ultimate_ability == 'TRUE' :
            if count_costs == 1: 
                insert_planeswalker(name, loyalty, is_passive, boost_ability, "-" , cost_ability)
            else:
                insert_planeswalker(name, loyalty, is_passive, boost_ability, cost_ability, high_cost_ability)
        else:
            insert_planeswalker(name, loyalty, is_passive, boost_ability, cost_ability, "-")
        ...
        boost_ability2 = boost_ability2.replace("']","")
        insert_planeswalker(name, loyalty, is_passive, boost_ability, boost_ability2, cost_ability)
...
def Single_Sided_Card(the_card):
    ...
    search_card = MTG_Cards(card_name, the_card.layout(), the_card.color_identity(), the_card.mana_cost(), regex_get_cmc(the_card.mana_cost()), the_card.type_line(), "", 'No', 'etb', 'ramp', 'draw', 'target', 'wipe',
        'activated', 'triggered', the_card.image_uris()['normal'], check_oracle_text(the_card.oracle_text()))

    if not search_card.clr_ID : search_card.clr_ID.append ('C')
    try:
        search_card.keywords = regex_get_keywords(str(the_card.__dict__.items()))
    except:
    ...
    if 'Creature' in search_card.c_type: insert_creature(card_name, sub_type, search_card.cmc, the_card.power(), the_card.toughness(), search_card.keywords)
    ...
    insert_card(card_name, search_card.clr_ID, search_card.mana_cost, search_card.cmc, search_card.c_type, search_card.keywords,
...
def Split_Card(the_card):
    ...
    card1 = cardfaces[0]
    card2 = cardfaces[1]
    search_card = MTG_FaceCards(card1['name'], the_card.layout(), card2['name'], the_card.color_identity(), card1['mana_cost'], card2['mana_cost'], regex_get_cmc(card1['mana_cost']),
                                regex_get_cmc(card2['mana_cost']), card1['type_line'], card2['type_line'], "", "", 'F:F', "etb1","etb2","ramp1","ramp2","draw1","draw2","target1","target2",
...
def Handle_Double(the_card, search_card):
    ...
    try:
        keywords = regex_get_keywords(str(the_card.__dict__.items()))
    ...
    if 'Planeswalker' in search_card.face2_type: Planeswalker_handle(search_card.flipname, search_card.face2_oracle_text, card2['loyalty'])
    ...
    if 'Creature' in search_card.face1_type:
        if '-' in search_card.face1_type:
            ...
            base_type = card_types[0]
            sub_type = card_types[1]
        insert_creature(search_card.name, sub_type, search_card.face1_cmc, card1['power'], card1['toughness'], search_card.face1_keywords)
    ...
    if 'Creature' in search_card.face2_type:
        ...
        insert_creature(search_card.flipname, sub_type, search_card.face2_cmc, card2['power'], card2['toughness'], search_card.face2_keywords)
    ...
    insert_face_card(search_card.name, search_card.layout, search_card.flipname, search_card.clr_ID, search_card.face1_cost, search_card.face1_cmc, search_card.face1_type, search_card.face1_keywords,
    ...
    insert_face_card(search_card.flipname, search_card.layout, search_card.name, search_card.clr_ID, search_card.face2_cost, search_card.face2_cmc, search_card.face2_type, search_card.face2_keywords,
...
convert_layout = {
    ...
    'double_faced_token' : Double_Faced_Card,
...
def qtest_search(card_name):  # just use this for doing scryfall lookups and printing to the console
    getcard = scrython.cards.Named(exact=card_name)
    # print ("all_parts: ", hasattr(getcard, 'all_parts'))
    # print ("card_faces", hasattr(getcard, 'card_faces'))
    # print ("colors: ", hasattr(getcard, 'colors'))
    # print (getcard.__dict__.items())
    # card_keywords = regex_get_keywords(str(getcard.__dict__.items()))
    try:
        card_keywords = regex_get_keywords(str(the_card.__dict__.items()))
    except:
        card_keywords = "[]"
